...
```{r, fig.height=3}
# Crear un bar plot de la frecuencia de cada categoría en la variable CATEGORIA
ggplot(CarreraLuz22F, aes(x = categoria)) + 
...
ggplot(CarreraLuz22F, aes(x = edad)) + 
...
```{r, fig.height=3}
# Crear un gráfico en 3D de la variable EDAD, TIEMPO_DE_CARRERA y CATEGORIA
plot_ly(CarreraLuz22F, x = ~edad, y = ~timerun, z = ~categoria, type = "scatter", mode = "markers", color = ~categoria) %>%
  add_markers() %>%
  layout(scene = list(xaxis_title = "Edad", yaxis_title = "Tiempo de Carrera", zaxis_title = "Categoría"))

```

### Distribucion de origenes de participantes

```{r, fig.height=3}
# Crear un gráfico de pastel de la variable NACIONALIDAD
ggplot(CarreraLuz22F, aes(x = "", y = 1, fill = origen)) + 
  geom_bar(width = 1, stat = "identity") +
  coord_polar("y", start = 0) +
  labs(title = "Gráfico de Pastel de Nacionalidad", 
       x = "", 
       y = "") +
...
### Analisis de la Carrera de la Luz 2022

**Analisis:** A continuacion, se presentan algunos resultados del analisis exploratorio de los datos:

* En la carrera existe una predominancia del genero masculino con un 61.29% de hombres frente a un 38.71% de mujeres 

* La mediana de edad es de 36 años

* La mayoria de los participantes son de nacionalidad Colombiana con un 99.6%

* Los tiempos de carrera entre categorias varian muy poco
